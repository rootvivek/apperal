...
function SignupPageContent() {
  ...
  // Prevent navigation to signup page if already on signup page
  useEffect(() => {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        ...
        const redirectTo = searchParams.get('redirect') || localStorage.getItem('signup-redirect') || '/';
        ...
        if (error) {
          ...
          setSuccess(true);
          setLoading(false);
          // Don't redirect - show success message with email verification instructions
        }
      } catch (err: any) {
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                  ...
                  <div className="mt-2 text-sm text-green-700">
                    {authMethod === 'email' ? (
                      <>
                        ...
                        <p className="mt-2">We&apos;ve sent a verification email with OTP and a verification link to <strong>{formData.email}</strong></p>
                        <p className="mt-1">Please check your email and click the verification link to activate your account.</p>
                        <p className="mt-1 text-xs">You can also sign in after verifying your email.</p>
                      </>
                    ) : (
                      <>
          ...
          {!success && authMethod === 'email' && (
            <div className="mt-6">
              ...
              <div className="mt-6 flex flex-col sm:flex-row items-center sm:justify-center gap-3 sm:gap-4">
                <button 
                  type="button"
                  onClick={handleGoogleSignIn}
                  className="w-full sm:w-auto inline-flex justify-center items-center py-2 px-4 sm:px-6 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors whitespace-nowrap">
                  ...
                  <span className="ml-2 sm:ml-3 font-medium truncate">Continue with Google</span>
                </button>

                <button 
                  type="button"
                  onClick={handleFacebookSignIn}
                  className="w-full sm:w-auto inline-flex justify-center items-center py-2 px-4 sm:px-6 border border-[#1877F2] rounded-md shadow-sm bg-[#1877F2] text-sm font-medium text-white hover:bg-[#0C63D4] hover:border-[#0C63D4] transition-colors whitespace-nowrap">
                  <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                  <span className="ml-2 sm:ml-3 font-medium truncate">Continue with Facebook</span>
                </button>
              </div>
            </div>
          )}
