...
import { ProductImageZoomDialog } from '@/components/ProductImageZoomDialog';
import { useImageZoom } from '@/hooks/product/useImageZoom';
import { useImageSwipe } from '@/hooks/product/useImageSwipe';
import { getBadgeStyle } from '@/utils/product/badgeStyles';
import { PLACEHOLDER_IMAGE } from '@/constants/productCard';

const PLACEHOLDER_PRODUCT = PLACEHOLDER_IMAGE;
...
export default function ProductMedia({
  ...
  const {
    ...
    handleTouchMove,
    handleTouchEnd,
  } = useImageSwipe(selectedImage, setSelectedImage, totalImages);
  ...
  return (
    <div className="w-full overflow-visible">
      ...
      <div className="hidden md:flex gap-2 overflow-visible">
        ...
        <div className="flex-1 relative overflow-visible flex items-center justify-center" ref={imageContainerRef}>
          {/* Main Image */}
          <div 
            ref={mainImageRef}
          ...
          >
            ...
            {product.badge && (
              <div className="absolute top-0 left-0 z-30 pointer-events-none m-0">
                <span className={`px-3 py-2 text-[10px] sm:text-xs font-medium ${getBadgeStyle(product.badge)}`}>
                  {product.badge}
                </span>
              </div>
            )}
          ...
          {/* Zoomed Image Preview - Right Side (Same size as main image) */}
          {showZoomPreview && imageContainerSize.width > 0 && (
            <div 
              className="hidden md:block absolute bg-white rounded-lg shadow-2xl z-50 overflow-hidden border border-gray-200"
              style={{
                width: `${imageContainerSize.width}px`,
                height: `${imageContainerSize.height}px`,
                left: 'calc(100% + 16px)', // 16px gap from main image
                top: '0'
              }}
            >
              <ImageWithFallback
        ...
        >
          {product.images.map((image, index) => (
                  <button
                    key={image.id}
                    onClick={() => setSelectedImage(index)}
                    className={`flex-shrink-0 w-14 h-14 sm:w-20 sm:h-20 rounded transition-all duration-200 shadow-sm flex items-center justify-center bg-white border ${
                      selectedImage === index 
                        ? 'scale-105 border-brand/50' 
                        : 'border-gray-200 hover:border-gray-300 hover:shadow-md'
                    }`}
