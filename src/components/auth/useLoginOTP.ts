...
export function useLoginOTP(): UseLoginOTPReturn {
  ...
  // Normalize phone number: add +91 prefix internally
  const normalizePhoneNumber = useCallback((phoneDigits: string): string => {
  ...
  }, [phone, sendOTP, normalizePhoneNumber]);
  ...
  }, [phone, secondsLeft, sendOTP, normalizePhoneNumber]);
  ...
  const verifyOtp = useCallback(async () => {
    ...
    try {
      ...
      if (verifyError) {
        ...
        // Success - state will be reset when modal closes
        setIsVerifying(false);
      }
    } catch (err: any) {
  ...
  }, [phone, otp, verifyOTP, normalizePhoneNumber]);
